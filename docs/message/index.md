...
- **消息管理员**: 负责消息系统的配置管理，包括分类管理、模板管理等
...
#### 4.1.2 消息分类聚合 (MessageCategory Aggregate)

**聚合根**: 消息分类 (MessageCategory)
**包含实体**: 无
**包含值对象**: 无
**一致性边界**: 保证分类信息的一致性
**核心业务规则**:
- 分类名称在同一所有者同一业务线下必须唯一
- 禁用状态的分类不能用于创建消息
- 分类删除时需要检查是否存在关联的消息
...
#### 4.2.2 消息分类 (MessageCategory) - 聚合根
...
#### 4.2.3 消息模板 (MessageTemplate) - 聚合根
...
- **分类权限验证**: 只有启用状态的分类才能用于创建消息，禁用分类的消息创建请求将被拒绝
...
#### 4.6.3 消息分类仓库接口 (MessageCategoryRepositoryInterface)

**持久化职责**: 消息分类实体的持久化存储和管理操作
...
#### 5.1.2 消息分类应用服务 (MessageCategoryApplicationService)

**功能描述**: 消息分类的管理服务，负责分类的创建、配置和权限管理
**核心方法**:
- create(): 创建消息分类
- update(): 更新分类配置
- enable(): 启用分类
...
#### 5.6.2 消息分类转换器 (MessageCategoryTransformer)

**数据映射职责**: 负责将分类命令数据转换为分类实体
...
基础设施层主要负责技术实现细节，包括外部服务集成、消息队列、缓存等技术组件。

### 6.1 外部服务集成

#### 6.1.1 推送服务适配器

**功能描述**: 集成第三方推送服务，提供统一的推送接口
**核心组件**:
- APP推送适配器（如JPush、个推等）
- 邮件服务适配器（如SendGrid、阿里云邮件等）
- 短信服务适配器（如阿里云短信、腾讯云短信等）

#### 6.1.2 消息队列集成

**功能描述**: 使用消息队列实现异步消息处理
**核心组件**:
- Redis队列：用于高频低延迟的消息处理
- RabbitMQ：用于可靠性要求高的消息处理
- 死信队列：处理失败的消息
...
#### 7.1.2 消息分类控制器 (MessageCategoryController)

**功能描述**: 提供消息分类的管理接口，支持分类的创建、配置和权限管理
**核心接口**:
- index(): 获取分类列表
- show(): 获取分类详情
- store(): 创建新分类
...
- 关联信息: category、sender、template
- 配置信息: channels、is_urgent、is_burn_after_read

#### 7.2.2 消息分类资源 (MessageCategoryResource)

**功能描述**: 消息分类数据的API资源转换
**核心字段**:
- 基础信息: id、name、description、icon、color、status
- 业务信息: biz、sort
...
**验证规则**:
- 必填字段: biz、receiver_id、title、content
- 字段格式: title最大255字符、content不能为空
- 业务验证: category_id必须存在且启用、receiver_id必须有效
- 推送配置: channels数组验证、expires_at时间验证

#### 7.3.2 消息分类请求验证 (MessageCategoryRequest)

**验证规则**:
- 必填字段: name、biz
- 唯一性验证: name在同一owner同一biz下唯一
- 业务验证: biz必须是有效的业务线
...
详细的存储层设计请参考：<!--@include: ./storage.md-->
