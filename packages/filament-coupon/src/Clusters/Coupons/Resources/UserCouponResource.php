...
class UserCouponResource extends Resource
{
    ...
    {
        return __('red-jasmine-filament-coupon::coupon.labels.user_coupon');
    }

    public static function getPluralModelLabel(): string
    {
        return __('red-jasmine-filament-coupon::coupon.labels.user_coupons');
    }
                ...
                Forms\Components\Section::make()
                    ->schema([
                        ...
                        Forms\Components\DateTimePicker::make('issue_time')
                            ->label(__('red-jasmine-filament-coupon::coupon.fields.issue_time'))
                            ->default(now())
                            ->required(),
                            
                        Forms\Components\DateTimePicker::make('expire_time')
                            ->label(__('red-jasmine-filament-coupon::coupon.fields.expire_time'))
                            ->required(),
                            
                        Forms\Components\DateTimePicker::make('used_time')
                            ->label(__('red-jasmine-filament-coupon::coupon.fields.used_time'))
                            ->visible(fn (Forms\Get $get) => $get('status') === UserCouponStatusEnum::USED->value),
                            
                        Forms\Components\TextInput::make('order_id')
                            ->label(__('red-jasmine-filament-coupon::coupon.fields.order_id'))
                            ->numeric()
                            ->visible(fn (Forms\Get $get) => $get('status') === UserCouponStatusEnum::USED->value),
                    ])
                    ->columns(2),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('status')
                    ->label(__('red-jasmine-filament-coupon::coupon.fields.status'))
                    ->badge()
                    ->color(fn (UserCouponStatusEnum $state) => match ($state) {
                        UserCouponStatusEnum::AVAILABLE => 'success',
                        UserCouponStatusEnum::USED => 'warning',
                        UserCouponStatusEnum::EXPIRED => 'danger',
                    })
                    ->sortable(),
                ...
                Tables\Columns\TextColumn::make('expire_time')
                    ->label(__('red-jasmine-filament-coupon::coupon.fields.expire_time'))
                    ->dateTime()
                    ->sortable(),
                ...
                Tables\Columns\TextColumn::make('coupon.discount_amount_value')
                    ->label(__('red-jasmine-filament-coupon::coupon.fields.discount_amount_value'))
                    ->formatStateUsing(fn ($state, $record) => 
                        $record->coupon->discount_amount_type === 'percentage' 
                            ? $state . '%' 
                            : '¥' . $state
                    )
                    ->sortable(),
                    
                Tables\Columns\TextColumn::make('coupon.threshold_value')
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('use')
                    ->label(__('red-jasmine-filament-coupon::coupon.actions.use'))
