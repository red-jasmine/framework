...
return new class extends Migration {
    public function up() : void
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->bigInteger('total_product_amount')->default(0)->comment('总商品金额');
            $table->bigInteger('total_tax_amount')->default(0)->comment('总税费');
            $table->bigInteger('discount_amount')->default(0)->comment('订单优惠');
            $table->bigInteger('freight_fee_amount')->default(0)->comment('订单运费');
            $table->bigInteger('payable_amount')->default(0)->comment('买家应付金额');
            $table->bigInteger('payment_amount')->default(0)->comment('买家实付金额');
            $table->bigInteger('refund_amount')->default(0)->comment('买家退款金额');
            // 结算
            $table->bigInteger('receivable_amount')->default(0)->comment('卖家应收金额');
            $table->bigInteger('received_amount')->default(0)->comment('卖家实收金额');
            $table->bigInteger('total_commission_amount')->default(0)->comment('总佣金');
            $table->bigInteger('total_platform_fee_amount')->default(0)->comment('平台服务费');
